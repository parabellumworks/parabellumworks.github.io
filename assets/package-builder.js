(() => {
  ...
  const CONFIG = {
    ...
    services: [
      ...
      { id: 'brand', number: '06', title: 'Marka Kimliği', description: 'Markanız için ortak bir görsel dil.', billing: 'once', visual: { src: 'brand-identity', alt: 'EIGHTFOLD Marka Kimliği hizmeti ürün görseli' }, info: { what: 'Markanın logo, renk, tipografi ve temel görsel sistemini oluşturur veya mevcut kimliği profesyonelleştiririz.', fit: 'Yeni başlayan, yenilenen veya dağınık görünen markalar için.', scope: 'Logo yönü, renk ve tipografi sistemi dahil; kapsamlı uygulamalar ayrıca netleştirilir.' }, options: [
        { id: 'mini', label: 'Mini', name: 'Mini Marka Kimliği', price: 14900 },
        { id: 'full', label: 'Tam', name: 'Tam Marka Kimliği', price: 24900 }
      ] },
      { id: 'production', number: '07', title: 'Fotoğraf & Video', description: 'İşletmenize özel içerik çekimi.', billing: 'once', unit: 'çekim', visual: { src: 'content-production', alt: 'EIGHTFOLD İçerik Üretimi hizmeti ürün görseli' }, info: { what: 'Sosyal medya kullanımına yönelik fotoğraf ve video üretimi yaparız.', fit: 'Mekânını, ürününü veya hizmetini güçlü görsellerle göstermek isteyen işletmeler için.', scope: 'Çekim süresi seçilen seçeneğe göre belirlenir; özel prodüksiyon ihtiyaçları ayrıca fiyatlandırılır.' }, options: [
  ...
  const renderServiceCard = service => `
    <article class="builder-card" data-service="${service.id}" data-billing="${service.billing}">
      <button class="builder-select" type="button" data-select="${service.id}" aria-pressed="false" aria-labelledby="builder-title-${service.id}" aria-describedby="builder-price-${service.id}">
        ...
        ${service.visual ? `<span class="builder-product-preview"><img src="/assets/products/${service.visual.src}-640.webp" alt="${service.visual.alt}" width="640" height="800" loading="lazy" decoding="async"></span>` : ''}
